...
/**
 * Lands here after Google/YouTube authorization. Exchanges the auth code for
 * tokens via the Edge Function, then redirects to Settings.
 */
export default function YouTubeCallback() {
  ...
  useEffect(() => {
    ...
    if (error) {
      setStatus('error');
      setMessage(`Google denied the request: ${error}. You can try again from Settings.`);
      return;
    }
    if (!code || !state) {
      setStatus('error');
    ...
    (async () => {
      ...
      if (result.ok) {
        setStatus('success');
        setMessage('YouTube connected! Redirecting...');
    ...
    <div className="min-h-screen bg-ink flex items-center justify-center px-6">
      <div className="max-w-md w-full bg-void border border-slate rounded-2xl p-8 text-center">
        ...
        <h1 className="font-display font-black text-2xl text-pearl mb-2" style={{ letterSpacing: '-0.02em' }}>
          {status === 'working' && 'Connecting YouTube'}
