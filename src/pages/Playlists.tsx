...
// Mixd gradient set — Signal Pink, Cobalt, Acid Lime, Deep Pink
const GRADIENTS = [
    ...
    <div className="max-w-4xl">
      ...
      {playlists.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {playlists.map((pl, i) => (
            <div key={pl.id} className="bg-[#0B0B12] border border-[#1A1A28] hover:border-[#FF2D87]/30 rounded-xl overflow-hidden transition cursor-pointer group">
              ...
              <div className="p-4">
                ...
                <p className="text-xs text-[#5E5E7A]">{pl.tracks} tracks</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#0B0B12] border border-[#1A1A28] rounded-xl w-full max-w-md">
            ...
            <div className="flex gap-3 p-5 border-t border-[#1A1A28]">
              ...
              <button onClick={handleCreate} disabled={!newName.trim()} className="flex-1 py-2.5 bg-[#FF2D87] hover:bg-[#E01570] disabled:opacity-40 rounded-lg text-sm font-semibold transition">Create</button>
            </div>
          </div>
        </div>
      )}
