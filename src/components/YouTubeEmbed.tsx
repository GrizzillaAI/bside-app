...
const YouTubeEmbed = forwardRef<YouTubeEmbedRef, YouTubeEmbedProps>(
  ({ videoId, isPlaying, volume, onStateChange, onTimeUpdate, onReady }, ref) => {
    ...
    return (
      <div
        className={`
          relative rounded-lg overflow-hidden bg-black transition-all duration-300 ease-in-out cursor-pointer shrink-0
          ${expanded
            ? 'w-80 h-[180px] z-50 shadow-2xl shadow-black/60'
            : 'w-20 h-[60px]'
          }
        `}
